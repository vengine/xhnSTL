...
namespace xhn {
                                                           ...
                                                           xhn::vector<xhn::static_string>&)>& isInheritFromClassProc)
    {
        ...
        char mbuf[1024];
        snprintf(mbuf, 1013, "/// this file is automatic generated by xhnSTL, do not manually modify it!!!\n");
        ...
        bridgeFile += "@property (assign) ActorAgent* (^createAgentProc)(void*, void*);\n";
                ...
                if (StrClassDecl == node->type && StrPublic == node->access) {
                    if (isInheritFromClassProc(node->name, StrSceneNodeAgent, inheritPath)) {
                        ...
                        if (isInheritFromClassProc(node->name, StrActorAgent, inheritPath)) {
                            ...
                            char mbuf[512];
                            snprintf(mbuf, 511,
                                     "    s_createActorAgentProcDic[@%c%s%c] = [[CreateActorAgentProc alloc] initWithProc:^(void* renderSys, void* skin)\n"
                                     "    {\n"
                                     "        %s* ret = [[%s alloc] initWithActor:[[VActor alloc] initWithVRenderSystem:renderSys skin:skin]];\n",
                                     '"', node->name.c_str(), '"', node->name.c_str(), node->name.c_str());
                            bridgeFile += mbuf;
                            int i = 0;
                            while (inheritPath.size()) {
